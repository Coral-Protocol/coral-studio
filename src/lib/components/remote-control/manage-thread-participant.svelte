<script lang="ts">
	...
	async function manageParticipant() {
		if (selectedId === undefined || participantName === undefined) return;
...
<Select.Root type="single" disabled={selectedId === undefined} bind:value={participantName}>
	<Select.Trigger class="w-[280px]"
		>{participantName === '' ? 'Select participant' : participantName}</Select.Trigger
	>
...
<Button
	disabled={selectedId === undefined || participantName === undefined}
	onclick={manageParticipant}>{remove ? 'Remove participant' : 'Add participant'}</Button
>
